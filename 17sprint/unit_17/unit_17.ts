...
let a1_res: number[] = [],
    ...
    a8_res: number[] = [],
    a11_res: number[] = [],
    a15_res: { name: string; pnum: string }[] = [];
...
let a3: (number | string)[] = [4, "3", 6, 7, "12", 34, "56", 78, 90, 11];
...
let a4: (number | string)[] = [4, "3", 6, 7, "12", 34, "56", 78, 90, 11];

function t4(): number[] {
    a4_res = a4.filter((item) => typeof item === 'number') as number[];
    return a4_res;
}
...
let a5: (number | string)[] = [3, 14, 15, 92, '6'];

function t5(): number[] {
    a5_res = a5.filter((item) => typeof item === 'number' && item % 2 === 0) as number[];
    return a5_res;
}
...
let a6: (number | string)[] = [3, 14, 15, 92, "6", "5", "hello", 32];

function t6(): number[] {
    a6_res = a6.filter((item) => typeof item === 'number' && item > 14) as number[];
    return a6_res;
}
...
let a7: string[] = ['Alto`s Adventure', 'Angry Birds 2', 'Anno 2205', 'Assassin`s Creed Chronicles'];

function t7(): string[] {
    a7_res = a7.map((item) => item.toLowerCase());
    return a7_res;
}
...
let a8: number[] = [3, 14, 15, 92, 7, 32, 59];
...
let a9: string[] = ['Quantum Break', 'Gears of War 4', 'Mass Effect: Andromeda', 'Far Cry Primal'];

function t9(): string[] {
    a9.forEach((item, index) => (a9[index] = item.toLowerCase()));
    return a9;
}
...
let a10: number[] = [2, 13, 14, -7, 9, 5, 0, -2, 14];
...
let a11: number[] = [2, 13, 14, -7, 9, 5, 0, -2, 14];

function t11(): number[] {
    a11_res = a11.filter((item, index) => index % 2 === 0);
    return a11_res;
}
...
let a13: number[] = [2, 13, 14, -7, 9, 5, 0, -2, 14];
let a13_num: number = 9;

function t13(): boolean {
...
let a14: string[] = ['c', 'C', 'd', 'e', 'E'];
let a14_sym: string = 'e';

function t14(): boolean {
    const lowerCaseArray = a14.map((item) => item.toLowerCase());
    return lowerCaseArray.includes(a14_sym.toLowerCase());
}
...
function t15(): User[] {
    a15_res = a15.filter((item) => item.pnum.length === 6);
    return a15_res;
}
