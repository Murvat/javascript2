// Task 1
// Есть массив d1, кнопка b-1, input i-1. Напишите функцию f1, которая при нажатии читает i-1 и добавляем его в конец массива. После чего выводит массив в .out-1. Поскольку мы будем выводить массив d1 неоднократно, то давайте вывод массива сделаем отдельной функцией, showArr - она подготовлена. Изучите ее - это хороший способ оптимизировать код.
...
function f12() {
    let res = -1; // 1. Создать переменную res равную -1
    let n = +document.querySelector('.i-12').value; // 2. Получить переменную n - число из input
    for (let i = 0; i < d12.length; i++) { // 3. Запустить цикл по массиву d12
        if (n === d12[i]) { // 4. Если введенное число n равно текущему элементу массива, то res = i; и выйти из цикла
            res = i;
            break;
        }
    }
...
function f14() {
    let n = +document.querySelector('.i-14').value; // 2. Получить переменную n - число из input
    for (let i = 0; i < n; i++) {
        d14.push(1);
    }
    showArr('.out-14', d14);
